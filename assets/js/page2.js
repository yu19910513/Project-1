...
function getStateImages() {
  const url2 = `https://pixabay.com/api?q=${
    stateName + "+-jones+-michael+-flag+-map+-oz"
  }&key=21438663-60940dce2a3b8f288719617da&lang=en&image_type=all&orientation=horizontal&safesearch=true&per_page=10&category=nature,science,education,places,animals,sports,buildings`;
...
function getStateButterfly(stateName, fact) {
  fetch(
    ...
    .then(function (data) {
      ...
      stateButterflyEl.textContent =
        "This state does not have a state butterfly";
    });
}
